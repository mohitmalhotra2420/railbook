...
import { parseDatePhrase, parseStatusDate, type DateHit } from "./dates";
import {
...
import { routeRailwayIntent } from "./toolRoute";
import { isNamedTrainCompare } from "./compare";
import { preferLocalFactIntent } from "./facts";
...
const CLASS_MAP: Array<{ re: RegExp; code: ClassCode }> = [
  ...
  { re: /\b(3\s*e|3e|ac 3 economy|third economy)\b/, code: "3E" },
...
const BERTH_MAP: Array<{ re: RegExp; value: string }> = [
  ...
  { re: /\b(coupe|koopay|koope|कूपे|कूप)\b/, value: "Coupe" },
  { re: /\b(cabin|kebin|केबिन)\b/, value: "Cabin" },
];
...
function extractClasses(t: string, lastAsked: DialogSlot): { classCodes?: ClassCode[]; acOnly?: boolean } {
  ...
  if (spoken) {
    if (!(lastAsked === "seat" && spoken === "SL" && !/sleeper|sl class/i.test(t))) {
      codes.push(spoken);
    }
  }
  ...
  if (/\b(general|gn|ur|gs|unreserved)\b/.test(t) && !codes.includes("2S")) codes.push("2S");
  if (lastAsked === "class" && /^(3|तीन|थ्री)$/i.test(t.trim()) && !codes.includes("3A")) {
    codes.push("3A");
  }
  if (codes.length) return { classCodes: [...new Set(codes)] };
  if (/\bac\b/.test(t) || /\bac\s*mein\b/.test(t) || /एसी/.test(t)) return { acOnly: true };
  return {};
}
...
function extractPassengers(t: string, lastAsked: DialogSlot): number | undefined {
  ...
  for (const [w, n] of Object.entries(NUM_WORDS)) {
    ...
    const re = new RegExp("(?:^|[\\s])" + w + "\\s*(?:" + TICKET_UNIT.source + ")", "i");
...
function extractPair(t: string): {
  ...
  if (originBit && destBit) {
    ...
    const origin = matchStation(oClean) || findStationsInText(originBit[1]).at(-1);
    const dest = matchStation(dClean) || findStationsInText(destBit[1]).at(-1);
    if (origin && dest && origin.code !== dest.code) return { from: origin, to: dest };
    const oU = !origin && (isClusterCityName(oClean) || looksLikePlace(oClean)) ? titlePlace(oClean) : undefined;
    const dU = !dest && (isClusterCityName(dClean) || looksLikePlace(dClean)) ? titlePlace(dClean) : undefined;
    if ((origin || oU) && (dest || dU) && (origin?.code || oU) !== (dest?.code || dU)) {
      return { from: origin, to: dest, unresolvedFrom: origin ? undefined : oU, unresolvedTo: dest ? undefined : dU };
    }
  }
  const to = t.match(
...
export function destCue(t: string): boolean {
...
export function originCue(t: string): boolean {
...
/** “Kro na trains check” — continue search, do not re-extract cities. */
export function isSearchNudge(text: string): boolean {
  const t = normalizeUtterance(text);
  if (!t) return false;
  if (/\b(pnr|live|fare|wallet|booking|seat(?:s)?\s*(?:hai|hain|available|batao))\b/.test(t)) return false;
  return (
    /\b(kro na|karo na|kar do na)\b/.test(t) ||
    /\btrains? check\b/.test(t) ||
    /\bcheck (karo|kardo|kar do)(?:\s+trains?)?\b/.test(t) ||
    /\b(search|dikhao) trains?\b/.test(t)
  );
}

function sameCity(a?: string, b?: string): boolean {
  if (!a || !b) return false;
  const ca = canonicalClusterCity(a) ?? a.trim().toLowerCase();
  const cb = canonicalClusterCity(b) ?? b.trim().toLowerCase();
  return Boolean(ca && cb && ca === cb);
}

/**
 * NVIDIA often puts “Mujhe Lucknow jaana hai” as origin and re-emits a
 * cluster city after the user already picked a station. Local dest/origin
 * cues win for slot polarity; search nudges drop invented cities.
 */
export function mergeNlu(ai: NluResult | undefined, local: NluResult, text: string, lastAsked: DialogSlot = null): NluResult {
  if (!ai) return local;
  const t = normalizeUtterance(text);
  if (isSearchNudge(text) && !clusterStationsForText(text) && !findStationsInText(text).length) {
    return {
      ...local,
      intent: local.intent === "NONE" || local.intent === "CONFIRM_YES" ? "SEARCH_TRAIN" : local.intent,
      from: undefined,
      to: undefined,
      unresolvedFrom: undefined,
      unresolvedTo: undefined,
      date: ai.date ?? local.date,
      passengerCount: ai.passengerCount ?? local.passengerCount,
    };
  }

  const out: NluResult = { ...ai };
  const dest = destCue(t);
  const origin = originCue(t);

  if (dest && !origin && lastAsked !== "from") {
    const aiHasOrigin = Boolean(out.from || out.unresolvedFrom);
    const aiHasDest = Boolean(out.to || out.unresolvedTo);
    if (aiHasOrigin && !aiHasDest) {
      const city =
        local.unresolvedTo ||
        out.unresolvedFrom ||
        out.from?.city ||
        out.from?.name ||
        local.to?.city;
      const locked = out.from;
      delete out.from;
      delete out.unresolvedFrom;
      if (city && clusterStations(city).length >= 2) {
        out.unresolvedTo = city;
        delete out.to;
      } else if (local.to) {
        out.to = local.to;
      } else if (locked) {
  ...
  if (origin && !dest && lastAsked !== "to") {
    const aiHasDest = Boolean(out.to || out.unresolvedTo);
    const aiHasOrigin = Boolean(out.from || out.unresolvedFrom);
    if (aiHasDest && !aiHasOrigin) {
      const city = local.unresolvedFrom || out.unresolvedTo || out.to?.city || local.from?.city;
      const locked = out.to;
      delete out.to;
      delete out.unresolvedTo;
      if (city && clusterStations(city).length >= 2) {
        out.unresolvedFrom = city;
      } else if (local.from) {
        out.from = local.from;
      } else if (locked) {
        out.from = locked;
  ...
  if (local.from && !out.from) out.from = local.from;
  if (local.to && !out.to) out.to = local.to;
  if (local.unresolvedFrom && !out.from && !out.unresolvedFrom) out.unresolvedFrom = local.unresolvedFrom;
  if (local.unresolvedTo && !out.to && !out.unresolvedTo) out.unresolvedTo = local.unresolvedTo;
  if (!out.date && local.date) out.date = local.date;
  if (out.passengerCount == null && local.passengerCount != null) out.passengerCount = local.passengerCount;
  if ((out.intent === "NONE" || !out.intent) && local.intent !== "NONE") out.intent = local.intent;
  if (isNamedTrainCompare(text) && local.intent === "COMPARE_TRAINS") out.intent = "COMPARE_TRAINS";
  out.intent = preferLocalFactIntent(out.intent, local.intent);
  if (local.trainNumber && !out.trainNumber) out.trainNumber = local.trainNumber;

  if (sameCity(out.unresolvedFrom, out.unresolvedTo)) {
    if (dest && !origin) delete out.unresolvedFrom;
    else if (origin && !dest) delete out.unresolvedTo;
  }

  return groundNlu(out, text);
}
...
function extractIntentKind(t: string): UserIntent {
  ...
  if (
    /कौन.?कौन|kaun.?kaun|which (cities|stations)|kaunse (shehar|shahr|city|station)|shehar hain|शहर हैं|stations (hain|list)|supported (cities|stations)|tumhare paas|आपके पास.*(शहर|स्टेशन)|koi bhi (city|shehar|shahr)|any city|kisi bhi (city|shehar)|saari cities|demo (cities|list)|poori (city |cities |shehar )?list|पूरी (लिस्ट|लिस्ट|city list)|full list|all cities/.test(
      t,
    )
  ) {
    return "LIST_CITIES";
  ...
  if (
    /\b(\d{5})\b/.test(t) &&
    /\b(history|kal ki journey|completed journey|yesterday|yasterday|ysterday)\b/.test(t)
  ) {
    return "TRAIN_HISTORY";
  }
  if (/लाइव|स्टेटस/.test(t) && /यस्टरडे|येस्टरडे|यस्टर्डे/.test(t)) {
    return "TRAIN_HISTORY";
  }
  if (
    /\b(live status|running status|live running|track train|train (ka )?status|status pata|iska live|uska live)\b/.test(t) ||
    /लाइव|स्टेटस|चल रही/.test(t)
  ) {
  ...
  if (/\b(jo best|best wala|recommend|best train|better train|kaunsi better)\b/.test(t)) return "SELECT_BEST";
  ...
  if (/\b(train list|kaunsi train|trains? dikhao|search train)\b/.test(t) || isSearchNudge(t)) return "SEARCH_TRAIN";
  return "NONE";
}
...
function applyCorrection(
  ...
  if (nahiLead && (known.from || known.to || known.date || known.passengerCount)) {
    ...
    const place = matchStation(cleanPlace(rest)) || findStationsInText(rest).at(-1);
    if (place && hasSe) {
    ...
    const clusterName = cleanPlace(rest);
    if (!place && isClusterCityName(clusterName) && hasSe) {
      return { correction: true, unresolvedFrom: titlePlace(clusterName), to: known.to ?? undefined };
    }
    if (!place && isClusterCityName(clusterName) && hasJaana && !hasSe) {
      return { correction: true, unresolvedTo: titlePlace(clusterName), from: known.from ?? undefined };
    }
  }
  ...
  if (!leftSt && isClusterCityName(left) && rightSt) {
    const codes = new Set(clusterStations(left).map((s) => s.code));
    if (known.from && codes.has(known.from.code)) return { correction: true, from: rightSt };
    if (known.to && codes.has(known.to.code)) return { correction: true, to: rightSt };
    if (lastAsked === "from") return { correction: true, from: rightSt };
    if (lastAsked === "to") return { correction: true, to: rightSt };
    return { correction: true, from: rightSt };
  }
  if (leftSt && rightSt) {
...
export function understand(text: string, ctx: NluContext = {}): NluResult {
  ...
  if (topic === "CHECK_PNR" || topic === "VIEW_BOOKINGS" || topic === "VIEW_WALLET" || topic === "ADD_MONEY"
    ...
    || topic === "CHECK_AVAILABILITY" || topic === "CHECK_FARE" || topic === "COMPARE_TRAINS"
    || topic === "SELECT_TRAIN" || topic === "GENERAL_RAILWAY_KNOWLEDGE") {
    const stationHit = uniqueStations(t)[0] ?? (trainNo ? undefined : matchStation(t.replace(/station board|live at station|platform pe trains|station pe trains|station par trains/gi, "").trim()));
    const statusDate =
      topic === "LIVE_TRAIN_STATUS" || topic === "TRAIN_HISTORY" ? parseStatusDate(raw, now) : undefined;
    return {
      intent: topic,
  ...
  if (!from && !unresolvedFrom && !to && !unresolvedTo) {
    const cluster = clusterStationsForText(t);
    if (cluster) {
      if (destCue(t) && !originCue(t)) unresolvedTo = cluster.city;
      else if (originCue(t) && !destCue(t)) unresolvedFrom = cluster.city;
      else if (lastAsked === "from") unresolvedFrom = cluster.city;
...
/** Drop station slots the utterance never actually said (AI/fuzzy false hits). */
export function groundNlu(n: NluResult, text: string): NluResult {
  if (n.correction) return n;
  const next: NluResult = { ...n };
  const junkTok = text
    .split(/\s+/)
    .map((w) => w.replace(/[?,.!]+/g, ""))
    .find((w) => isGarbageStationQuery(w));
  if (next.from && !stationGroundedInText(next.from, text)) {
    delete next.from;
    if (!next.unresolvedFrom && junkTok) next.unresolvedFrom = junkTok;
  }
  if (next.to && !stationGroundedInText(next.to, text)) {
    delete next.to;
    if (!next.unresolvedTo && junkTok) next.unresolvedTo = junkTok;
  }
  if (junkTok && !next.from && !next.unresolvedFrom) next.unresolvedFrom = junkTok;
  return next;
}
