...
2. A DIRECT train from the user's origin with a FRESH seat is preferred over a CONNECTION. A same-train earlier-stop / book-upto ticket (kind bfe) is the SAME physical journey (same train, traveller boards at the same station) — do NOT rank it below a from-origin ticket automatically. Compare bfe vs direct on: seat quality for the party (AVL with known count > AVL count-unknown > RAC), travel time, and TOTAL fare for the party (a much cheaper class with a real seat is better value than an expensive 1A/2A just because its ticket starts at the origin). Prefer the from-origin ticket only when the two are otherwise comparable (it avoids the small extra fare from the earlier station).
...
4. Then speed (shorter duration), sensible departure time, fewer changes, then cost/class. 4b. TRANSPARENCY: in "why" you MUST name the closest runner-up (the next-best option with a real seat) and state the deciding factor(s) — seat / time / fare / class — with the actual numbers from the sheet (e.g. "13042 Himgiri 40 min fast hai lekin LDH se WL; PGW se 2A AVL 6 @ ₹1960 — agar 1A mehnga lage to ye lo"). Never hide a faster or cheaper seated option.
