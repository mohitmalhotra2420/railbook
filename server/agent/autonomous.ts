...
function systemPrompt(today: string, state: AutoAgentState, protocol: "tools" | "json"): string {
  ...
  const lines = [
    ...
    `8b. SEAT RULE: seat/berth/class/availability ka sawaal SAARE trains par ("2A me kaunsi train me seat hai", "AC trains dikhao", "sabse sasti seat wali", "raat 9 ke baad sleeper me seat", "sirf confirmed wali", "12029 me seat hai kya") → PEHLE findSeats call karo (classCode, afterText, sortBy khud set karo; onlyAvailable=true SIRF jab user ne khud "available/khali/sirf available/confirmed seat" maanga ho — warna false taaki WL/N-A trains bhi aayein). Seat ke number/status memory se kabhi mat likho. WL ka confirm% kabhi mat batao — sirf WL number. Jawab: SAARI trains ki lines (number, naam, class, status+count, fare) — AVL/RAC aur WL/N-A dono, jab tak user ne sirf-available nahi maanga. Kabhi mat likho "baaki trains kisi card me hain".`, 
