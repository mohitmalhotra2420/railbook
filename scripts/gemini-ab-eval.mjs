/**
 * Shadow A/B: NVIDIA (production) vs Gemini (eval only).
...
import { extractWithGemini } from "../server/understand/gemini.ts";
...
async function runProvider(kind, row) {
  ...
  for (let i = 0; i < 3; i++) {
    const out = await extractWithGemini(input);
    if (out.failureReason !== "http_429") return out;
    const wait = 25000;
    console.error(`EVAL gemini_backoff_s=${wait / 1000}`);
...
async function main() {
  ...
  if (!geminiReady) console.error("EVAL gemini_key=missing");
  else console.error("EVAL gemini_key=present");
  console.error(`EVAL gemini_model=${process.env.GEMINI_MODEL || "gemini-2.5-flash"}`);
  ...
  const lines = [];
  lines.push("# Gemini vs NVIDIA A/B report");
  lines.push("");
  lines.push(`Date: 2026-08-23 (Asia/Calcutta). Cases: ${rows.length}.`);
  lines.push("");
  lines.push("**Mode:** Gemini is SHADOW / EVALUATION only. NVIDIA remains the production default.");
  lines.push("Gemini output is never applied to the customer reply, never runs tools, never books, never debits the wallet.");
  lines.push("");
  lines.push("| Provider | Role | Model |");
  lines.push("|---|---|---|");
  lines.push(`| NVIDIA | production / primary | ${process.env.NVIDIA_MODEL || "openai/gpt-oss-20b"} |`);
  lines.push(`| Gemini | shadow / eval | ${process.env.GEMINI_MODEL || "gemini-2.5-flash"} |`);
  lines.push("");
  ...
  writeFileSync(new URL("../GEMINI_VS_NVIDIA_AB_REPORT.md", import.meta.url), `${lines.join("\n")}\n`);
  console.error("EVAL report=GEMINI_VS_NVIDIA_AB_REPORT.md");
  console.error(
    JSON.stringify({
