...
13. **Station-choice follow-up dead-end (user-reported, live screenshot)** — "amritsar se delhi jaana hai" ke station-clarification ke baad user ka "4"/"NDLS" reply: (a) deterministic fallback EMPTY reply deta tha, (b) app conflicting "Waise booking continue… Kahan jaana hai?" bubble aata tha, (c) model kabhi-kabhi apni knowledge se options likhta tha (invented "NDL"). Fixes: station-pick resolver (numbered/compact/paren/pairs formats, 4 formats + API-verified), pre-fill dono engines ke liye, stationPicked prompt flag (confirm nahi, seedha answer), resume-bubble suppression jab reply khud sawaal ho, grounding token-boundary fix ("NDL" ≠ "NDLS" substring), needs_choice/ambiguity relay real options ke saath, unhelpful-summary override. 440/440 tests; live 3/3 verified.
